import { prisma } from '../web/lib/db';
import { runTenderAiAnalyze } from '../web/lib/use-cases/tender-ai-analyze';

const tenderId = 'cmn5j5rj7000dtwk8noi3tiwt';

const tender = await prisma.tender.findUnique({
  where: { id: tenderId },
  include: { company: { include: { users: { take: 1, include: { user: true } } } } }
});

if (!tender || !tender.company || !tender.company.users || !tender.company.users[0]) {
  throw new Error('tender/company-user not found');
}

const cu = tender.company.users[0];
const res = await runTenderAiAnalyze(
  { user: { id: cu.user.id, email: cu.user.email }, companyId: tender.companyId },
  tenderId
);

console.log(JSON.stringify(
  res.ok ? { ok: true, analysisId: res.analysis.id, status: res.analysis.status } : res,
  null,
  2
));

await prisma.$disconnect();
